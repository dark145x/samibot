...
usages: "facebook cover photo make by Kawsar",
cooldowns: 0,
dependencies: {
    "fs-extra": "",
    "request": "",
...
if ((this.config.credits) != "Kawsar") { return api.sendMessage(`[ WARNING ] - Itz Kawsar Don't credits modules ${this.config.name} was changed to ${this.config.credits} bởi ADMINBOT ${global.config.BOTNAME} 😐 Stop now!!!`, event.threadID, event.messageID)}
  ...
  if(!args[0]) return api.sendMessage('🔶Hey Itz Me Arfhat🌸\n➡️Please enter primary name!!!', threadID, messageID)
  else return api.sendMessage(`🔷Itz Arfhat🌸\n➡️You chose the main name: ${args.join(" ").toUpperCase()}\n\n(Reply to this message and choose your secondary name)`,event.threadID, (err, info) => {
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    case "tenphu": {
      ...
      return api.sendMessage(`🌸Itz Arfhat🐰\n➡️chose the sub-name ${event.body.toUpperCase()}\n\n(Reply to this message enter your phone number)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "sdt",
          name: `fbcover`,
          author: senderID,
    ...
    case "sdt": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`🌸It's Arfhat🪄\n➡️You have selected SDT as: ${event.body.toUpperCase()}\n\n(Reply this message to enter your email)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "email",
          name: `fbcover`,
          author: senderID,
    ...
    case "color": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`🌺It's Arfhat🌺\n🔶You have selected the address as : ${event.body.toUpperCase()}\n➡️Reply this message to enter your background color(enter no as default color)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "create",
          name: `fbcover`,
          author: senderID,
    ...
    case "create": {
      ...
      api.unsendMessage(handleReply.messageID);
      api.sendMessage(`🌺Hey Itz Me Arfhat Your Cover Almost Done Please Wait⏳ Initializing the imager!`,threadID, (err, info) => {
      setTimeout(() => {
