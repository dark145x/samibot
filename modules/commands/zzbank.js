...
module.exports.run = async ({ event, api, Currencies, args, Users, permssion }) => {
  ...
  if (args[0] == '-r' || args[0] == 'register') {
    ...
    else return api.sendMessage(`[ WARNING ] ➜ You already have an account on the system Hadi Bank🏦`, threadID, messageID)
  }
  if (args[0] == "loan") {
    ...
    if (!dataJson.find(i => i.senderID == senderID)) { return api.sendMessage(`[ WARNING ] ➜ You do not have an account on the system yet Hadi Bank🏦\n[ ${timeNow} ]`, threadID) }
  ...
  if (args[0] == 'check' || args[0] == 'coins') {
    ...
    return api.sendMessage({
      body: "🏦⚜️𝗦𝗔𝗞𝗜𝗕𝗜𝗡 𝗕𝗔𝗡𝗞⚜️🏦\n•━━━━━━━━━━━━━━━━•\n®️➜ register - register Hadi bank\n💹➜ trade - deposit money Hadi bank to make profit\n🏧➜ cashout - withdraw money from Hadi bank\n🧾➜ check - show information banking\n💸➜ pay - transfer money to others\n💰➜ loan - take loan (not recommended)\n💵➜ repay - repay loan",
      attachment: t
    }, threadID)
  }
}
  ...
  try {
   ...
   const { readFileSync, writeFileSync, unlinkSync, createReadStream } = require("fs-extra");
    ...
    if (handleReaction.type == "trade") {
      ...
      return api.sendMessage(`[ SUCCESS ] ➜ You have sent ${replace(handleReaction.send)}$ enter Hadi Bank\n💷 Interest: +${laisuat * 100}% in ${timeIM / 60} minute\n[ ${timeNow} ]`, event.threadID)
    }
    if (handleReaction.type == "rut") {
      userData.money = parseInt(userMoney) - parseInt(handleReaction.send);
      writeFileSync(pathData, JSON.stringify(dataJson, null, 4), "utf-8");
      await Currencies.increaseMoney(event.userID, parseInt(handleReaction.send))
      return api.sendMessage(`[ SUCCESS ] ➜ You have withdrawn ${replace(handleReaction.send)}$ from Hadi Bank\n[ ${timeNow} ]`, event.threadID)
    }
