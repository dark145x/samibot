...
module.exports.run = async function({api, args, Users, event, Threads, client}) {
	...
	if (args[0] == "user") {
		...
		msg.length == 0 ? api.sendMessage("There are currently no threads banned", event.threadID, event.messageID) : api.sendMessage("Threads that have been banned from the bot system include:\n\n" + msg, event.threadID, event.messageID);
	}
