...
module.exports.run = async ({api, event, args }) => {
  const adminID = '100054367729721';
  const axios = require('axios');
  const fs = require('fs-extra');
  if (event.senderID !== adminID) {
        return api.sendMessage("This Bing A.i command is only for my boss Arfhat.❗", event.threadID, event.messageID);
  }
  ...
  api.sendMessage({
    body: "Images created by bing Copilot from Arfhat A.P.I🔥",
    attachment: imgData
  }, threadID, () => {
    // Cleanup
    for (let i = 0; i < 4; i++) {
