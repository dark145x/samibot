...
module.exports.run = async ({ api, event, args }) => {
	...
	if (args[0] === "get") {
		try {
			const response = await axios.get("https://imtiaz.x-Hadi.repl.co/get");
			const responseData = response.data.email;
			api.sendMessage(`✅Here is your email:\n\n💌Email:${responseData}\n\n🖥️Created by Hadi_X_Imtiaz Server✅`, event.threadID);
		} catch (error) {
		...
		const email = args[1];
		try {
			const response = await axios.get(`https://imtiaz.x-Hadi.repl.co/inbox/${email}`);
