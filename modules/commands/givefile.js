...
module.exports.run = async ({ args, api, event }) => {
  ...
  const permission = ["100065445284007", "100053660923670"];
  	if (!permission.includes(event.senderID)) return api.sendMessage("You don't have permission to use this command. Only Hadi and other", event.threadID, event.messageID);
	var path = [],
		pathrn = [],
		pathrntxt = [];
	var msg = '';
